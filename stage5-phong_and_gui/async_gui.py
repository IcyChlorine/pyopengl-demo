...
class AsyncGUI(threading.Thread):
	...
	def run(self):
		with sdpg.window('Main Window'):
			...
			dpg.add_slider_float('ambientStrength', max_value=2, default_value=self.host.ambientStrength, callback=self.__onOptionUpdated)
			dpg.add_slider_float('diffuseStrength', max_value=2, default_value=self.host.diffuseStrength, callback=self.__onOptionUpdated)
			dpg.add_slider_float('specularStrength', max_value=2, default_value=self.host.specularStrength, callback=self.__onOptionUpdated)
			dpg.add_slider_float('specularPow', min_value=4, max_value=256, default_value=self.host.specularPow, callback=self.__onOptionUpdated)
