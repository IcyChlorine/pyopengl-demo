...
class Application:
	def __init__(self, **kwargs):
		...
		self.isOptionUpdatedAsync = False
		self.optionUpdatedAsync = set()
	...
	def mainLoop(self):
		if self.isOptionUpdatedAsync:
			if 'renderSkeleton' in self.optionUpdatedAsync:
				if self.renderSkeleton:
					glPolygonMode(GL_FRONT_AND_BACK, GL_LINE)
				else:
					glPolygonMode(GL_FRONT_AND_BACK, GL_FILL)
				self.optionUpdatedAsync.remove('renderSkeleton')
			self.isOptionUpdatedAsync = False
	...
	def setOptionAsync(self, optionName, optionValue):	# maybe called asynchronously
		...
		self.isOptionUpdatedAsync = True
		self.optionUpdatedAsync.add(optionName)
		#eval(f'self.{optionName}={str(optionValue)}')
