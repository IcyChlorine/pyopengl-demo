...
class Camera():
	...
	def __updateMatrix(self):
		...
		cameraPos[1] = self.ρ*np.cos(self.θ)
		r            = self.ρ*np.sin(self.θ)
		print(r)
		cameraPos[0] =      r*np.sin(self.φ)
		cameraPos[2] =      r*np.cos(self.φ)
		self.viewMat = np.array(
			glm.lookAt(
				glm.vec3(cameraPos),
				glm.vec3(cameraLookAt-cameraPos),#vector forward
				glm.vec3(cameraUp)
			)
		)#有优化的空间